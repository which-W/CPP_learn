...
struct myVector {
    ...
    size_t m_size;
    size_t m_cap;
    [[no_unique_address]] Alloc m_alloc;//原先的Alloc如果为空那么会对齐内存把一个字节的内存变为8个字节和m_size等变量对齐使用[[no_unique_address]]就不会出现内存对齐而产生的消耗
    //使用int来对其内存
    ...
    myVector(std::initializer_list<T> ilist, Alloc const& alloc = Alloc())
        : myVector(ilist.begin(), ilist.end(), alloc) {}



    explicit myVector(size_t n, Alloc const& alloc = Alloc()) : m_alloc(alloc) {
    ...
    myVector(size_t n, T const& val, Alloc const& alloc = Alloc()) : m_alloc(alloc) {
        m_data = m_alloc.allocate(n);
        m_cap = m_size = n;
        for (size_t i = 0; i != n; i++) {
    ...
    myVector(InputIt first, InputIt last, Alloc const& alloc = Alloc()) : m_alloc(alloc) {
        size_t n = last - first;
        m_data = m_alloc.allocate(n);
        m_cap = m_size = n;
        for (size_t i = 0; i != n; i++) {
    ...
    myVector(myVector&& that) noexcept : m_alloc(std::move(that.m_alloc)) {
        m_data = that.m_data;
        m_size = that.m_size;
        m_cap = that.m_cap;
        that.m_data = nullptr;
    ...
    myVector(myVector&& that, Alloc const& alloc) noexcept : m_alloc(alloc) {
        m_data = that.m_data;
        m_size = that.m_size;
        m_cap = that.m_cap;
        that.m_data = nullptr;
    ...
    void swap(myVector& that) noexcept {
    ...
    myVector(myVector const& that) : m_alloc(that.m_alloc) {
    ...
    myVector(myVector const& that, Alloc const& alloc) : m_alloc(alloc) {
        m_cap = m_size = that.m_size;
        if (m_size != 0) {
            m_data = m_alloc.allocate(m_size);
            for (size_t i = 0; i != m_size; i++) {
    ...
    bool operator==(myVector const& that) noexcept {
        return std::equal(begin(), end(), that.begin(), that.end());
    }

    auto operator<=>(myVector const& that) noexcept {
